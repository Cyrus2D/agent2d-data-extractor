...
//            ADD_ELEM("p_t", convertor_angle(ball_pos.th().degree()));
    ...
    // player
    {
        ...
        for (uint i = 0; i < players.size(); i++) {
            ...
            DataSide side = player->side() == wm.ourSide() ? TM : OPP;
            extract_base_data(player, side);
            extract_pos3(player, wm, side, new_self_pos);
            extract_vel(player, side);
            extract_counts(player, side);
...
void DataExtractor::init_file(const rcsc::WorldModel &wm) {
    ...
    std::string dir = "/mnt/f/2d/agent2d-data-extractor/src/data/";
...
void DataExtractor::extract_pass_angle(const AbstractPlayerObject *player, const WorldModel &wm, DataSide side) {
    ...
    for (int o = 1; o <= 11; o++) {
        ...
        opp_dist_body_diff.push_back(
                std::make_pair(opp->pos().dist(ball_pos), ((ball_pos - opp->pos()).th() - opp->body()).abs()));
        ...
        opp_pass_projection_bodydiff.push_back(
                std::make_pair(proj_pos.dist(opp->pos()), ((proj_pos - opp->pos()).th() - opp->body()).abs()));
    }
    if (option.openAnglePass == side || option.openAnglePass == BOTH) {
...
void DataExtractor::extract_pos2(const AbstractPlayerObject *player, const WorldModel &wm, DataSide side) {
    if (option.pos == side || option.pos == BOTH) {
        ADD_ELEM("pos_x", convertor_x(player->pos().x));
        ADD_ELEM("pos_y", convertor_y(player->pos().y));
    }
    ...
    Vector2D rpos = player->pos() - wm.interceptTable()->fastestTeammate()->pos();
    if (option.relativePos == side || option.relativePos == BOTH) {
        ADD_ELEM("kicker_x", convertor_dist_x(rpos.x));
        ADD_ELEM("kicker_y", convertor_dist_y(rpos.y));
        ADD_ELEM("kicker_r", convertor_dist(rpos.r()));
...
void DataExtractor::extract_pos3(const AbstractPlayerObject *player,
                                 const WorldModel &wm,
                                 DataSide side,
                                 const Vector2D new_self_pos) {
    Vector2D player_pos;
    if (wm.self().unum() == player->unum() && wm.self().side() == player->side())
        player_pos = new_self_pos;
    else
        player_pos = player->pos();

    if (option.pos == side || option.pos == BOTH) {
        ADD_ELEM("pos_x", convertor_x(player_pos.x));
        ADD_ELEM("pos_y", convertor_y(player_pos.y));
    }
    if (option.polarPos == side || option.polarPos == BOTH) {
        ADD_ELEM("pos_r", convertor_dist(player_pos.r()));
        ADD_ELEM("pos_t", convertor_angle(player_pos.th().degree()));
    }
    Vector2D rpos = player_pos - wm.interceptTable()->fastestTeammate()->pos();
    if (option.relativePos == side || option.relativePos == BOTH) {
        ADD_ELEM("kicker_x", convertor_dist_x(rpos.x));
        ADD_ELEM("kicker_y", convertor_dist_y(rpos.y));
        ADD_ELEM("kicker_r", convertor_dist(rpos.r()));
    ...
    if (option.in_offside == side || option.in_offside == BOTH) {
        if (player_pos.x > wm.offsideLineX()) {
            ADD_ELEM("pos_offside", 1);
        } else {
            ADD_ELEM("pos_offside", 0);
        }
